...
def upload_report():
    ...
    try:
        ...
        # Get patient details
        c.execute('SELECT name FROM patients WHERE email = ?', (session['user_id'],))
        patient = c.fetchone()
        if not patient:
        ...
        report_id = f"report_{timestamp}_{random_string}"
        
        # Verify the ID is unique
        while True:
            c.execute('SELECT id FROM patient_records WHERE id = ?', (report_id,))
            if not c.fetchone():
                break
            # If ID exists, generate a new random string
            random_string = ''.join(random.choices('ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', k=6))
            report_id = f"report_{timestamp}_{random_string}"
        ...
        if 'xray' in request.files:
            ...
            if xray_file and xray_file.filename:
                # Save X-ray image with unique filename
                filename = secure_filename(f"{session['user_id']}_{report_id}_xray.jpg")
                filepath = os.path.join(app.config['UPLOAD_FOLDER'], filename)
                xray_file.save(filepath)
                
                # Process X-ray with AI model
                xray_data = {
                    'image_path': filepath,
                    'tb_probability': random.uniform(0.1, 0.9),  # Replace with actual AI analysis
                    'infected_areas': [
                        {'x': random.randint(50, 150), 'y': random.randint(50, 150), 'radius': random.randint(10, 30)}
        ...
        sputum_data = None
        if 'sputum_id' in request.form:
            sputum_data = {
                'test_id': request.form['sputum_id'],
                'result': request.form['sputum_result'],
                'date': request.form['sputum_date'],
                'notes': request.form.get('sputum_notes', '')
            }
        ...
        for key in request.form:
            if key.startswith('symptom_'):
                symptoms_data[key] = request.form[key]
        ...
        report_data = {
            ...
            'type': 'xray' if xray_data else 'sputum' if sputum_data else 'symptoms',
            'xray_data': xray_data,
        ...
        c.execute('''
            INSERT INTO patient_records (
                id, patient_email, report_type, report_data, status, created_at
            ) VALUES (?, ?, ?, ?, ?, ?)
        ''', (
            report_id,
            session['user_id'],
...
def get_analysis(report_id):
    ...
    return jsonify({
        'success': True,
        'analysis': patient_records[report_id]['analysis']
    })
